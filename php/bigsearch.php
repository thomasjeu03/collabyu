<?php
    ...
    AND(user_name_USERS LIKE '%{$searchTerm}%' 
    OR user_username_USERS LIKE '%{$searchTerm}%' 
    OR user_profil_USERS LIKE '%{$searchTerm}%'
    OR user_localisation_USERS LIKE '%{$searchTerm}%'
    OR user_instrument_USERS LIKE '%{$searchTerm}%'
    ) ORDER BY RAND() ");
    if(mysqli_num_rows($sql) > 0){
        include "bigsearchData.php";
    }else{
